...
1. The `input` field MUST be an object when present
2. Fields within `input` MUST be validated against the resource schema
3. Unknown fields within `input` SHOULD be rejected with a `VALIDATION_UNKNOWN_FIELD` error
4. Identifier parameters MUST NOT appear inside `input`
