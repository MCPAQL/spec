...
- **`autonomy_pause`:** Generated when the Autonomy Evaluator returns `continue: false`. Provides the reason and contributing factors. For `verify` tier pauses, MUST include `verificationId` in metadata. For `confirm` tier pauses, cleared when `confirm_operation` is called for the paused action.
