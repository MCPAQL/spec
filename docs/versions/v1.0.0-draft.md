...
This broad scope is what makes the safety dongle deployment model possible (see the Execution Safety Loop specification): a standalone MCP-AQL server can function as a universal safety layer for all tool calls in a client session, regardless of which server handles the actual operation.
...
- The execution lifecycle operations (`execute_agent`, `record_execution_step`, `complete_execution`) are either unavailable or operate as no-op progress tracking without safety enforcement
...
{
  "capabilities": {
    "execution_safety_loop": false
  }
}
```

Adapters MAY support partial modes between full enforcement and fully disabled:

- **Monitoring mode:** Actions are logged and evaluated, but `continue` is always `true` (advisory only)
- **Logging mode:** Actions are recorded for audit purposes without evaluation
