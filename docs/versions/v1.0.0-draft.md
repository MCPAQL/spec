...
| `run_job` | EXECUTE | Starts runtime process |
...
| EXECUTE | `run` (initiate), `cancel` (terminate) | Lifecycle operations |
...
| `run` / `cancel` | POST | Lifecycle control |
...
- `run_job` (not `execute_job`)

Operations that act on the system as a whole (rather than a specific resource) MAY omit the resource suffix (e.g., `introspect`, `search`).
...
- Map the operation to its canonical equivalent in introspection metadata
