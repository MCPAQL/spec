...
| `confirm_operation` | EXECUTE | Confirm a Gatekeeper-blocked operation (Section 8.7.3) |
| `verify_challenge` | CREATE | Submit out-of-band verification code for Danger Zone unblocking (Section 8.8) |
...
| 31-60 | `confirm` | Pause for human review |
| 61-85 | `verify` | Pause + create verification challenge |
| 86-100 | `danger_zone` | Hard stop + out-of-band verification (Section 8.8) |
...
interface AgentNotification {
  ...
  metadata?: {
    ...
    verificationId?: string;     // For danger_zone blocks
  };
...
- **`autonomy_pause`:** Generated when the Autonomy Evaluator returns `continue: false`. Provides the reason and contributing factors.
...
When the Autonomy Evaluator assigns a `danger_zone` safety tier (Section 8.7.2, Stage 4), the system MUST escalate beyond a simple pause. Out-of-band verification requires a human to provide a verification code through a channel that the AI cannot access, creating cryptographic proof that a human reviewed and approved the action.

#### 8.8.1 Trigger Conditions

Out-of-band verification is triggered when:

- The safety tier evaluation assigns `danger_zone` (risk score 86-100), OR
- The `nextActionHint` matches a hard-block (`deny`) pattern configured in the adapter (Section 8.7.2, Stage 3)

When triggered, the `AutonomyDirective` MUST return `stopped: true` and SHOULD include a `danger_zone` notification broadcast to all executing agents.
