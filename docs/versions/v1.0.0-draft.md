...
│  │  CRUDE or adapter-defined semantic endpoints         │   │
│  └──────────────────────────────────────────────────────┘   │
│                          │                                  │
│  ┌──────────────────────────────────────────────────────┐   │
│  │              Operation Router                        │   │
...
**Semantic Endpoint Mode (adapter-defined endpoint families)**
```
...
Other valid semantic endpoint families might be shaped around domains such as database access (`tables`, `queries`, `admin`) or hardware integrations (`inventory`, `telemetry`, `control`).
...
Each operation MUST be assigned exactly one standard semantic category based on its characteristics. In semantic-endpoint modes (`crude` or adapter-defined `semantic` families), each operation MUST also be routed to exactly one exposed endpoint family. Implementations MUST reject operations sent to incorrect endpoints in semantic-endpoint modes.
...
- MUST support at least one endpoint mode (`crude`, adapter-defined `semantic`, or `single`)
...
- MUST reject operations sent to wrong endpoints in semantic-endpoint modes

**Level 2: Full Conformance**
- MUST meet Level 1 requirements
- MUST support both a semantic-endpoint mode (`crude` or adapter-defined `semantic`) and `single` mode
