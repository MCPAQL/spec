...
- When `stopped` is `true`, the agent has been hard-blocked (Danger Zone — Section 8.8). The agent MUST cease all actions until unblocked
...
interface AutonomyDirective {
  ...
  stopped?: boolean;              // True if hard-blocked (Danger Zone)
...
Implementations MUST include `continue` and `factors` in every `AutonomyDirective`. All other fields are OPTIONAL.
...
- **`permission_pending`:** Generated when the Gatekeeper denies an operation that requires confirmation. Cleared when `confirm_operation` is called for the blocked operation.
- **`autonomy_pause`:** Generated when the Autonomy Evaluator returns `continue: false`. Provides the reason and contributing factors. For `verify` tier pauses, SHOULD include `verificationId` in metadata. For `confirm` tier pauses, cleared when `confirm_operation` is called for the paused action.
- **`danger_zone`:** Broadcast to ALL executing agents when any agent triggers a Danger Zone block. Includes the `verificationId` for the affected agent.
...
| `deny` | string[] | Glob patterns that hard-block with no confirmation path |
...
- The initial `AutonomyDirective` MUST include the challenge ID via a notification: `autonomy_pause` with `metadata.verificationId` for `verify` tier, or `danger_zone` with `metadata.verificationId` for `danger_zone` tier
...
When an agent is blocked by a Danger Zone event:
...
Adapters MUST NOT allow an agent to bypass a Danger Zone block by starting a new execution. The block is applied at the agent level, not the execution level.

For `verify` tier challenges (where `stopped` is not `true`), the agent is paused rather than blocked. While a `verify` challenge is pending, subsequent `record_execution_step` calls for the same agent SHOULD return `continue: false` with the pending challenge ID in the reason field, until the challenge is completed or expires. Unlike Danger Zone blocks, `verify` pauses do not persist across server restarts and do not prevent starting new executions.
