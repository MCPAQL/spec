...
Any state except completed → cancelled (via cancel operation)
```
...
| `completed_at` | string (ISO 8601) | When execution finished |
...
| `total` | number | Total expected value (if known) |
