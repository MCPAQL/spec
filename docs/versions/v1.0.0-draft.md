...
- Confirmation tokens (unless explicitly persisted; see [Section 5.4](../security/confirmation-tokens.md#54-session-scoped-tokens) of the Confirmation Token Specification)
...
Adapters MUST invalidate all session-scoped state when a session ends, whether gracefully or abruptly.
...
- Client-provided metadata (via `_meta`)
