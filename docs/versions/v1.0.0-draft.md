...
Each operation MUST be assigned exactly one standard semantic category based on its characteristics. In semantic endpoints mode, each operation MUST also be routed to exactly one exposed endpoint family. Implementations MUST reject operations sent to incorrect endpoints in semantic endpoints mode.
...
- MUST support at least one endpoint mode (`semantic` or `single`)
...
- MUST reject operations sent to wrong endpoints in semantic endpoints mode

**Level 2: Full Conformance**
- MUST meet Level 1 requirements
- MUST support both `semantic` and `single` endpoint modes
...
| Semantic Endpoints Mode (CRUDE profile) | 5 | ~4,300 | ~85% |
